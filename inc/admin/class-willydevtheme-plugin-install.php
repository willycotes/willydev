<?php
/**
 * willydevtheme Plugin Install Class
 *
...
if ( ! class_exists( 'willydevtheme_Plugin_Install' ) ) :
	/**
	 * The willydevtheme plugin install class
	 */
	class willydevtheme_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'willydevtheme-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $willydevtheme_version, 'all' );

			wp_enqueue_style( 'willydevtheme-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $willydevtheme_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'willydevtheme-button', 'disabled' ),
					);
				...
				</span> <?php echo /* translators: conjunction of two alternative options user can choose (in missing plugin admin notice). Example: "Activate WooCommerce or learn more" */ esc_html__( 'or', 'willydevtheme' ); ?>
...
return new willydevtheme_Plugin_Install();
